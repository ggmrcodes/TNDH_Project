...
import HbTrendChart from '../../components/charts/HbTrendChart';
import SymptomDotPlot from '../../components/charts/SymptomDotPlot';
      ...
      (async () => {
        ...
        if (isMockMode) {
          const [txs, slogs, meds] = await Promise.all([
            ...
            mockServices.getSymptomLogs(user.id, 200),
            mockServices.getMedicationReminders(user.id),
          ]);
          if (!cancelled) {
            setTransfusions(txs);
            setLogs(slogs);
          ...
          const [txs, slogs] = await Promise.all([
            realTransfusionService.getTransfusions(user.id),
            realSymptomService.getSymptomLogs(user.id, 200),
          ]);
          if (!cancelled) {
            setTransfusions(txs);
            setLogs(slogs);
            setMedications([]); // No real medication service yet
          }
        }
